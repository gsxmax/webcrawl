...
def  getHtmlResponse(url):
	...
	headers = {
        ...
        rel = requests.get(url, headers=headers, timeout=30)
...
if __name__ == '__main__':
    ...
    while len(init_urls) > 0:
        ...
        if r is not None:
            ...
            print 'current Image urls is {0},the url count is {1}. '.  format(img_urls, count)
            downImageFromUrls(img_urls, img_path)
        else:
            url = init_urls.popleft()
            print 'current url is {0},the count is {1}. '.  format(url, count)
            time.sleep(0.01)
            continue
